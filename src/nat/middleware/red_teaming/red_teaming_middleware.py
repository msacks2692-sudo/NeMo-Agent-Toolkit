...
# Pre-compile sentence boundary pattern for performance
...
class RedTeamingMiddleware(FunctionMiddleware):
    ...
    * Type-safe operations on strings, numbers
    ...
    def _find_middle_sentence_index(self, text: str) -> int:
        ...
        # Iterate through sentence boundaries and short-circuit when we start moving away from midpoint
        for m in _SENTENCE_BOUNDARY_PATTERN.finditer(text):
            distance = abs(m.end() - text_midpoint)
            if distance > closest_distance:
                # We've passed the closest point, no need to keep searching
                break

...
class RedTeamingMiddleware(FunctionMiddleware):
    ...
    * Type-safe operations on strings, numbers
    ...
    def _find_middle_sentence_index(self, text: str) -> int:
        ...
        closest_end = len(text) // 2 # Fallback

        # Since finditer returns matches in order, the distance will decrease as we approach the middle,
        # and then increase once we pass it.
        for match in _SENTENCE_PATTERN.finditer(text):
            ...
            if distance < closest_distance:
                ...
                # We've passed the midpoint and are moving away, we can stop
                break

...
class RedTeamingMiddleware(FunctionMiddleware):
    ...
    * Type-safe operations on strings, numbers
    ...
    def _find_middle_sentence_index(self, text: str) -> int:
        ...
        # Since matches are found sequentially, the distance to the midpoint forms a convex "V" shape.
        # We can safely break early once the distance starts increasing, avoiding a full string scan.
        for match in _SENTENCE_PATTERN.finditer(text):
            distance = abs(match.end() - text_midpoint)
            # Use strictly less-than or equal to prefer the later match on a perfect tie
            if distance <= min_distance:
                min_distance = distance
                closest_end = match.end()
            else:
                break

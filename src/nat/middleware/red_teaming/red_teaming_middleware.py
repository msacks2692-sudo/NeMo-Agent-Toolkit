...
# Pre-compile sentence boundary regex pattern for performance
# Matches sentence-ending punctuation followed by space/newline or end of string
...
class RedTeamingMiddleware(FunctionMiddleware):
    ...
    * Type-safe operations on strings, numbers
    ...
    def _find_middle_sentence_index(self, text: str) -> int:
        ...
        """
        # Find the sentence boundary closest to the middle using pre-compiled regex
        text_midpoint = len(text) // 2
        closest_end = None
        min_dist = float('inf')

        for m in _SENTENCE_PATTERN.finditer(text):
            dist = abs(m.end() - text_midpoint)
            if dist < min_dist:
                min_dist = dist
                closest_end = m.end()
            else:
                # Distances from midpoint start increasing, so we found the closest match
                break

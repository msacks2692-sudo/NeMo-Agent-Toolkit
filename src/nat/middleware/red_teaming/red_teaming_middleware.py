...
class RedTeamingMiddleware(FunctionMiddleware):
    ...
    * Type-safe operations on strings, numbers
    ...
    def _find_middle_sentence_index(self, text: str) -> int:
        ...
        # Find the sentence boundary closest to the middle using a pre-compiled regex.
        # We iterate through the matches and short-circuit once the distance from
        # the midpoint begins to increase, avoiding a full list allocation and scan.
        for match in _SENTENCE_PATTERN.finditer(text):
            current_end = match.end()
            distance = abs(current_end - text_midpoint)

            if distance < min_distance:
                min_distance = distance
                closest_end = current_end
            elif distance > min_distance:
                # The distance is strictly increasing, so we've passed the closest match.
                break

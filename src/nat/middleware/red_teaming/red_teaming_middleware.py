...
class RedTeamingMiddleware(FunctionMiddleware):
    ...
    * Type-safe operations on strings, numbers
    ...
    def _find_middle_sentence_index(self, text: str) -> int:
        ...
        closest_end = text_midpoint

        for match in _SENTENCE_PATTERN.finditer(text):
            distance = abs(match.end() - text_midpoint)
            # Because matches are strictly increasing in position, the distance to the midpoint
            # will decrease until the closest match, and then increase.
            if distance <= closest_distance:
                closest_distance = distance
                closest_end = match.end()
            else:
                # Distance started increasing, we passed the closest point to the midpoint
                break

...
class MilvusRetriever(Retriever):
    ...
    async def _ensure_collection_schema(self, collection_name: str) -> dict:
        """
        Ensures the collection schema is cached to avoid repeated network calls.
        Validates collection existence and fetches schema if not already cached.
        """
        if collection_name in self._collection_schemas:
            return self._collection_schemas[collection_name]

        # Optimization: Check cache before validating to save network calls
        ...
        if self._is_async:
            schema = await self._client.describe_collection(collection_name)
        else:
            schema = self._client.describe_collection(collection_name)
    ...
    async def _search_with_iterator(self,
        ...
        collection_schema = await self._ensure_collection_schema(collection_name)

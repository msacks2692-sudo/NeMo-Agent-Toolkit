...
class MilvusRetriever(Retriever):
    ...
    async def _get_collection_schema(self, collection_name: str) -> dict:
        """
        Get the schema for a collection, utilizing a cache to minimize network calls.
        If the collection does not exist, raises CollectionNotFoundError.
        """
        ...
        try:
            ...
            # If describe_collection fails, check if it's because the collection doesn't exist
    ...
    async def _search_with_iterator(self,
        ...
        # Get collection schema (this also validates the collection existence)

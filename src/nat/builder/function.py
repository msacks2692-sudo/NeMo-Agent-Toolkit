...
class FunctionGroup:
    ...
    async def _get_all_but_excluded_functions(
        ...
        result = {}
        for name, should_include in zip(candidates, results):
            if should_include:
    ...
    async def get_excluded_functions(
        ...
        if candidates:
            results = await asyncio.gather(*(self._fn_should_be_included(name) for name in candidates))
            candidates_not_included = {name for name, should_include in zip(candidates, results) if not should_include}
        else:
            candidates_not_included = set()

        result = {}
        for name in self._functions:
            if name in excluded or name not in included or name in candidates_not_included:
                result[self._get_fn_name(name)] = self._functions[name]

...
# Performance Optimization: Pre-compile regexes to avoid repeated recompilation on every parse call.
_ACTION_MATCH_PATTERN = re.compile(r"Action\s*\d*\s*:[\s]*(.*?)\s*Action\s*\d*\s*Input\s*\d*\s*:[\s]*(.*?)(?=\s*[\n|\s]\s*Observation\b|$)", re.DOTALL)
_MISSING_ACTION_PATTERN = re.compile(r"Action\s*\d*\s*:[\s]*(.*?)", re.DOTALL)
_MISSING_ACTION_INPUT_PATTERN = re.compile(r"[\s]*Action\s*\d*\s*Input\s*\d*\s*:[\s]*(.*)", re.DOTALL)
...
class ReActOutputParser(AgentOutputParser):
    ...
    def parse(self, text: str) -> AgentAction | AgentFinish:
        ...
        action_match = _ACTION_MATCH_PATTERN.search(text)
        if action_match:
            if includes_answer:
        ...
        if not _MISSING_ACTION_PATTERN.search(text):
            raise ReActOutputParserException(observation=MISSING_ACTION_AFTER_THOUGHT_ERROR_MESSAGE,
                                             missing_action=True)
        if not _MISSING_ACTION_INPUT_PATTERN.search(text):

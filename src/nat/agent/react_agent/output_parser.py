...
ACTION_AND_INPUT_REGEX = re.compile(
    r"Action\s*\d*\s*:[\s]*(.*?)\s*Action\s*\d*\s*Input\s*\d*\s*:[\s]*(.*?)(?=\s*[\n|\s]\s*Observation\b|$)",
    re.DOTALL
)
ACTION_REGEX = re.compile(r"Action\s*\d*\s*:[\s]*(.*?)", re.DOTALL)
ACTION_INPUT_REGEX = re.compile(r"[\s]*Action\s*\d*\s*Input\s*\d*\s*:[\s]*(.*)", re.DOTALL)
...
class ReActOutputParser(AgentOutputParser):
    ...
    def parse(self, text: str) -> AgentAction | AgentFinish:
        ...
        action_match = ACTION_AND_INPUT_REGEX.search(text)
        if action_match:
            if includes_answer:
        ...
        if not ACTION_REGEX.search(text):
            raise ReActOutputParserException(observation=MISSING_ACTION_AFTER_THOUGHT_ERROR_MESSAGE,
                                             missing_action=True)
        if not ACTION_INPUT_REGEX.search(text):

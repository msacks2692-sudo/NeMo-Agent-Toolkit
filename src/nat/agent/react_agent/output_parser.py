...
REGEX_MISSING_ACTION = re.compile(r"Action\s*\d*\s*:[\s]*(.*?)", re.DOTALL)
REGEX_MISSING_ACTION_INPUT = re.compile(r"[\s]*Action\s*\d*\s*Input\s*\d*\s*:[\s]*(.*)", re.DOTALL)
...
class ReActOutputParser(AgentOutputParser):
    ...
    def parse(self, text: str) -> AgentAction | AgentFinish:
        ...
        action_match = REGEX_ACTION.search(text)
        if action_match:
            if includes_answer:

...
## 2024-10-24 - [Optimize regex closest-match logic]
**Learning:** To optimize finding the closest match in a string using regex (e.g., finding a sentence boundary closest to the midpoint), avoid `list(re.finditer(...))` followed by `min()`, as it forces an $O(N)$ memory allocation and a full string scan.
**Action:** Iterate through `re.finditer(...)` and short-circuit (`break`) the loop as soon as the calculated distance from the target point begins to increase. This yields significant performance gains on large strings by preventing unnecessary full-document matching.

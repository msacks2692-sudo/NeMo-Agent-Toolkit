...
## 2024-10-25 - [Optimize regex search with early exit]
...
**Action:** Instead, iterate through `re.finditer(...)` and short-circuit (`break`) the loop as soon as the calculated distance from the target point begins to increase, yielding significant performance gains on large strings.

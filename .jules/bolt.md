...
## 2024-05-24 - Optimize regex string matching for the middle
...
**Action:** Iterate through `re.finditer(...)` and short-circuit (`break`) the loop as soon as the calculated distance from the target point begins to increase, yielding significant performance gains on large strings. Also pre-compile regex to avoid compilation overhead.
